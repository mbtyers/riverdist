...
Allowing different route-detection algorithms: a definite time-saver
-----------------------------------------------------
...
Running `buildsegroutes()` calculates the routes to each network segment from a single point, defined as the river network mouth.  This allows route calculation to switch to an even simpler method, further reducing processing time.  The `buildsegroutes()` function also includes an option to calculate a distance lookup table as well.  This may take a few seconds to run, but will reduce computation times even further - cutting the already-fast segment route algorithm by 50-80%.  Lookup tables can be calculated directly using `buildlookup()`, but will be extremely slow to calculate without running `buildsegroutes()` first.
...
In this case, building segment routes and distance lookup tables takes about as much time as calculating a single distance, about a second.  Calculating a single distance afterwards takes about 80 microseconds using segment routes and about 25 microseconds using the lookup tables.  This dramatically saves time if multiple distances are to be computed, such as in a large matrix of distances, or multiple analyses.  Kernel density and K-function analysis is prohibitively slow otherwise.
