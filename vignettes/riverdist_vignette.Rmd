...
**Computing a matrix of network distances between all observations for an individual using `riverdistancematobs()`**

River distance can also be calculated between all observations of a single individual.  In the output matrix shown below for individual 1, the element with row identifier `1` and column identifier `3` represents the river distance from the location observed in survey 1 to the location observed in survey 3, 83.6 km.  An important note is that the distances reported are net distance between locations, and are not intended to be cumulative.

```{r}
riverdistancematobs(indiv=1, ID=smallset$id, survey=smallset$flight,
      seg=smallset$seg, vert=smallset$vert, rivers=Gulk, full=FALSE)
```

Specifying `full=TRUE` reports an output matrix that includes all observations, with values of `NA` if the individual was not observed.  This allows the output matrices for multiple individuals to have the same rows and columns, thus being directly comparable.

```{r}
riverdistancematobs(indiv=1, ID=smallset$id, survey=smallset$flight,
      seg=smallset$seg, vert=smallset$vert, rivers=Gulk, full=TRUE)
```
...
A matrix of the river network distance from every observation to every other observation can also be calculated using `riverdistancemat()`.  The `logical` argument can be used for subsetting, if the full network distance matrix is not needed.  This capability is shown below, calculating the distance matrix only for observations occurring on segment number 2.  The `ID` argument can be used with a vector of observation labels, to display row and column labels that might be more meaningful than observation indices.
...
A method has been provided to display kernel density calculated from point data, using river network distance.  The `riverdensity()` function calculates scaled kernel density at approximately regularly-spaced river network locations, with the linear resolution specified by the optional `resolution=` argument.  A gaussian (normal) kernel is used by default, but a rectangular (simple density) kernel can be used as well.  The `survey=` argument may also be used with a vector of survey identifiers corresponding to the point data.  If the `survey=` argument is used, `riverdensity()` will calculate separate densities for each unique survey, and `plotriverdensity()` will produce a separate plot for each unique survey.

The `plotriverdensity()` function can display densities using line thickness, color, or both.  For additional plotting arguments including possible color ramps, see `?plotriverdensity`.

Density plots are shown below for the first three flights of the fakefish dataset.
...
A few possible plotting styles are shown below.

```{r,fig.width=7.5,fig.height=6}
data(fakefish_density)
par(mfrow=c(2,3))
plotriverdensity(riverdensity=fakefish_density, ramp="grey", dark=0.85, maxlwd=15, 
                 whichplots=1)
plotriverdensity(riverdensity=fakefish_density, bycol=FALSE, denscol="black", 
                 linecol="grey", maxlwd=15, whichplots=1)
plotriverdensity(riverdensity=fakefish_density, ramp="red", dark=0.85, maxlwd=15, 
                 whichplots=1)
plotriverdensity(riverdensity=fakefish_density, ramp="stoplight", dark=0.85, maxlwd=15, 
                 whichplots=1)
plotriverdensity(riverdensity=fakefish_density, ramp="stoplight", dark=0.85, maxlwd=15, 
                 bylwd=FALSE, lwd=3, points=FALSE, whichplots=1)
plotriverdensity(riverdensity=fakefish_density, ramp="heat", dark=0.85, maxlwd=15, 
                 whichplots=1)
```
...
**Summarizing up-river position, defined as distance from mouth, using `mouthdist()` for one observation and `mouthdistobs()` for a dataset**

In some cases, a meaningful summary measure may be the up-river position of each observation.  Distance between an individual observation and the river mouth can be calculated using `mouthdist()`.  In the case of multiple observations of a set of individuals, a summary matrix can be calculated using `mouthdistobs()`, which returns a matrix of distances from the river mouth, with each row corresponding to a unique individual and each column corresponding to a unique survey.
...
**Plotting the summary matrix from `mouthdistobs()` or other distance sequence using `plotseq()`**

The resulting matrix of up-river position returned from `mouthdistobs()`, or another distance sequence returned from `riverdistanceseq()` or `upstreamseq()`, can be plotted using `plotseq()`.  A few types of plots are available, with two shown below.  Additional `type=` arguments can be seen by calling `?plotseq`.  
...
Depending on the study, `plotseq()` may be useful in plotting the upstream distance between observations of each individual, as calculated in `upstreamseq()`.  It is worth noting that the outputs from both `upstreamseq()` and `riverdistanceseq()` may have many empty cells, if individuals were missed on any flights.
...
**Plotting STUUUUUFFFFFFF**

The `plotupstreammatobs()` function is intended to summarize the outputs of `upstreammatobs()` for all individuals in a dataset.  The output is the upper triangle of a matrix of boxplots in which boxplot `[i,j]` represents the upstream distances traveled between observation `i` and observation `j`, for all individuals observed in those observations.  Each boxplot is overlayed with a horizontal line at an upstream distance of zero, for an illustration of up-river or down-river movement trend for that pairwise movement.
...
The example below shows how the Skilak Lake section of the Kenai River network was originally read from a shapefile, with the vertices of segment 71 overlayed. 
...
The same section is shown below after adding vertices every 200 meters.
