...
1. Import a linear shapefile using `line2network()`.  The shapefile to import must be a linear feature - converting a polygon to a line will result in an outline, which will not be useable.  The shapefile to import should be as simple as possible.  If GIS software is available, both trimming the shapefile to the same region as the study area and performing a spatial dissolve are recommended.
...
3. Convert point data to river locations using `xy2segvert()` or `ptshp2segvert()`.  This will snap each point location to the nearest river network locations.

4. Perform all analyses.
