...
River distance can also be calculated between all observations of a single individual.  In the output matrix shown below for individual 29, the element with row identifier `5` and column identifier `8` represents the river distance from the location observed in survey 5 to the location observed in survey 8, 75455m.  An important note is that the distances reported are net distance between locations, and are not intended to be cumulative.
