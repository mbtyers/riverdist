...
**A plotting method for the summary matrix from `mouthdistobs()` or other distance sequence using `plot()`**
...
**Plotting all upstream movements using `matobslist()` and `plotmatobslist()`**

In a more generalized application, the `matobslist()` function calculates the distances or upstream distances, between each pair of observation events, for all individuals.  Functionally, this is a summary of the outputs of `riverdistancematobs()` or `upstreammatobs()`, but for all individuals in a dataset.  The output from `matobslist()` can then be plotted using `plotmatobslist()`, providing a one-plot summary of all movements within a dataset.  It is recommended to use the default `method="upstream"` in `matobslist()`, which calculates directional (upstream) distances, which will result in a more informative plot.
